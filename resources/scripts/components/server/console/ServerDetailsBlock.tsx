...
const ServerDetailsBlock = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={classNames('grid grid-cols-6 gap-2 md:gap-4', className)}>
            ...
            <StatBlock icon={faHdd} title={'磁碟容量'} color={getBackgroundColor(stats.disk / 1024, limits.disk * 1024)}>
                <Limit limit={textLimits.disk}>{bytesToString(stats.disk)}</Limit>
            </StatBlock>
            <StatBlock icon={faCloudDownloadAlt} title={'網路 (接收流量)'}>
                {status === 'offline' ? <span className={'text-gray-400'}>離線</span> : bytesToString(stats.rx)}
            </StatBlock>
            <StatBlock icon={faCloudUploadAlt} title={'網路 (輸出流量)'}>
