...
const FileDropdownMenu = ({ file }: { file: FileObject }) => {
    ...
    return (
        <>
            ...
            >
                『<span className={'font-semibold text-gray-50'}>{file.name}</span>』 確認刪除此檔案？您將無法恢復檔案內容。
            </Dialog.Confirm>
            <DropdownMenu
                ref={onClickRef}
            ...
            >
                <Can action={'file.update'}>
                    ...
                    <Row onClick={() => setModal('chmod')} icon={faFileCode} title={'調整權限'} />
                </Can>
                {file.isFile && (
                ...
                {file.isArchiveType() ? (
                    <Can action={'file.create'}>
                        <Row onClick={doUnarchive} icon={faBoxOpen} title={'取消壓縮'} />
                    </Can>
                ) : (
